...
static int validate_ivt(struct ivt *ivt_initial)
{
	...
	printf("ivt_hdr->magic = 0x%x\n", (uint32_t)ivt_hdr->magic);
	printf("ivt_hdr->length = 0x%x\n",(uint32_t)ivt_hdr->length);
	printf("ivt_hdr->version = 0x%x\n",(uint32_t)ivt_hdr->version);
...
int authenticate_image(u32 ddr_start, u32 raw_image_size)
{
	...
	printf("hab auth ret = %d, img: %u size: %u ivt offset: %u tot sz: %u\n",
		ret, ddr_start, raw_image_size, ivt_offset, bytes);
	return ret;
}
